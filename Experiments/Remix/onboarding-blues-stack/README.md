...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create onboarding-blues-stack-a715
  fly apps create onboarding-blues-stack-a715-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app onboarding-blues-stack-a715
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app onboarding-blues-stack-a715-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'onboarding-blues-stack-a715-staging' does not match app name in config file 'onboarding-blues-stack-a715'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name onboarding-blues-stack-a715-db
  fly postgres attach --app onboarding-blues-stack-a715 onboarding-blues-stack-a715-db

  fly postgres create --name onboarding-blues-stack-a715-staging-db
  fly postgres attach --app onboarding-blues-stack-a715-staging onboarding-blues-stack-a715-staging-db
  ```
