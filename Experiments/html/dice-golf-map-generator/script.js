function generateMap(width, height) {
  const map = Array(height)
    .fill()
    .map(() => Array(width).fill(0)); // 0 = rough
  ...
  // Step 2: Clean up tiny isolated pockets of the rough
  // (simple flood fill or just skip if you like noisy maps)

  // Step 3: Place decorations (trees, rocks, flowers, signs)
  const decorations = [];

  // Add trees - preferring near the edges of the rough
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      if (map[y][x] === 1) {
        // Count rough neighbors
        let roughNearby = 0;
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            if (map[y + dy][x + dx] === 0) roughNearby++;
          }
        }

        // Higher chance near rough, lower in open fields
        const treeChance = roughNearby > 0 ? 0.09 : 0.012;
  ...
  //       x,
  //       y,
  //       type: ["rock", "bush"][Math.floor(Math.random() * 4)],
  //     });
  //   }
  // }
